...
def files_combination(file_path):           #将一个文件夹下的所有诗词句合并
	words=''
	i=1
...
def creat_all(name):
	path=r'C:\Users\lenovo\Desktop\bigwork\ziliao\\'+name
	words=files_combination(path)
	words=screen(words)
	create_cloud(words,stopwords_,name)
	c=create_chart(words,screen_words,name)
	return c
...
def lyric():
	...
	create_cloud(lyric,stopwords_,'xiandai')
	create_chart(lyric,screen_words,'xiandai')
